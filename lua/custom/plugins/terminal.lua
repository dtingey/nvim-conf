...
function M.setup()
  vim.keymap.set('n', '<leader>tsh', FloatingTerminal, { noremap = true, silent = true, desc = 'Toggle floating terminal' })
  ...
  vim.api.nvim_create_autocmd('TermEnter', {
    ...
    once = true,
    callback = function()
      -- After exiting insert mode (first <ESC><Esc>), allow the second <Esc> to close
      vim.keymap.set('n', '<Esc>', function()
        if terminal_state.is_open then
          vim.api.nvim_win_close(terminal_state.win, true)
          terminal_state.is_open = false
        end
      end, { buffer = terminal_state.buf, noremap = true, silent = true, desc = 'Close floating terminal with <Esc>' })
    end,
  })
